...
function SelfUid(){return jsclient.data.pinfo.uid}
function IsMyTurn()
{
...
function MJGang2Net(cd){jsclient.gamenet.request("pkroom.handler.tableMsg",{cmd:"MJGang",card:cd}); }
function MJChi2Net(pos){jsclient.gamenet.request("pkroom.handler.tableMsg",{cmd:"MJChi",pos:pos}); }
function MJHu2Net(){jsclient.gamenet.request("pkroom.handler.tableMsg",{cmd:"MJHu"}); }
function MJPeng2Net(){jsclient.gamenet.request("pkroom.handler.tableMsg",{cmd:"MJPeng"}); }
...
function CheckEatVisible(eat)
{
	...
	var vnode=[];
    if(tData.tState==TableState.waitPut&&pl.mjState==TableState.waitPut)//gang hu put
	{
		if(IsMyTurn())
		{
			 if(pl.isNew && mj.canHu(tData.noBigWin,pl.mjhand)>0)
			 {
				 vnode.push(eat.hu._node);
			 }
			 var rtn=leftCard>0?mj.canGang1(pl.mjpeng,pl.mjhand,pl.isNew):[];
			 if(rtn.length>0)
			 {
    ...
    else if(tData.tState==TableState.waitEat)
	{
		...
		if(!IsMyTurn())
		{
            ...
            var huType=mj.canHu(tData.noBigWin,pl.mjhand,tData.lastPut);
			if(huType>0)
			{
				var canHu=false;
...
function SetPlayerVisible(node,off)
{
	...
	if(pl)
	{
		...
		setOffline(node,off);
		//if (pl.mjpeng) {InitPlayerHandUI(node,off);} 
	...
	}else
	{
		...
		if(WxHead) WxHead.removeFromParent(true);
	}
}
...
function HandleNewCard(node,msg,off)
{
	...
	*/
	...
	{
		mylog("uids[tData.curPlayeroff"+off);
		if (off==0) {
...
function AddNewCard(node,copy,name,tag,off,specialTAG)
{
	...
	var cp=cpnode.clone();cp.visible=true;cp.name=name;
...
function TagOrder(na,nb){return na.tag-nb.tag}
...
function InitPlayerNameAndCoin(node,off)
{
	...
	var bind={
		head:{
			name:{
				_text:function(){ return unescape(pl.info.nickname||pl.info.name || "玩家");  }
			}
		}
	}
...
function setWxHead(node,d,off)
{
	if(d.uid==getUIHead(off).uid)
	{
		...
		sp.setName("WxHead");
		node.addChild(sp);
		doLayout(sp,[0.8,0.8],[0.5,0.5],[-0.01,0.04],false,true);
	}
}
...
var rePlayLayer=cc.Layer.extend({
	jsBind:{
		...
		{	
			...
			_event:{
				reinitSceneData:function(eD){ this.visible=CheckArrowVisible();SetArrowRotation(this)  }
		...
		wait:{
            ...
            _event:{
				reinitSceneData:function(eD){ this.visible=CheckInviteVisible();	},
		...
		down:{
			...
			head:
			{ 
			 zhuang:{_run:function()
						{
							this.visible =false;
						},_event:{
							waitPut:function(){showPlayerZhuangLogo(this,0);}
							,reinitSceneData:function(){if(CheckArrowVisible()) showPlayerZhuangLogo(this,0);}
						}
					},chatbg:{_run:function(){this.getParent().zIndex = 600;} ,chattext:{
						_event:{
					...
					fenghu:
					{
						...
						{
							initSceneData:function(){ checkFHLogo(this,0)}
							,roundEnd:function(){this.visible =false;}
							,mjhand:function(){this.visible =false;}
							,MJPeng:function(){checkFHLogo(this,0)}
						}
					}
				,_click:function(btn){ showPlayerInfo(0,btn); 	 }
			 	,_event:{
				  loadWxHead:function(d){	setWxHead(this,d,0);}
			...
			ready:{ _layout:[[0.07,0.07],[0.5,0.5],[0,-1.5]], 
                    ...
                    _event:{
						...
						,addPlayer:function(){ CheckReadyVisible(this,0); },removePlayer:function(){ CheckReadyVisible(this,0); }
			...
			out1:{_layout:[[0,0.08],[0.5,0],[-6.4,3.8]],_visible:false},
			out0:{_layout:[[0,0.08],[0.5,0],[-6.5,2.8]],_visible:false},
			effectStateAct:{
				_run:function () {
					this.zIndex=100;
				},
			...
			_event:{
				...
				reinitSceneData:function(eD){ SetPlayerVisible(this,0);},
				addPlayer:function(eD){ SetPlayerVisible(this,0);},removePlayer:function(eD){ SetPlayerVisible(this,0);},
		...
		right:{
			...
			head:{
					zhuang:{
						_run:function()
						{
							...
							,reinitSceneData:function(){if(CheckArrowVisible()) showPlayerZhuangLogo(this,1);}
						}
					...
					fenghu:
					{
						...
						{
							initSceneData:function(){ checkFHLogo(this,1)}
							,roundEnd:function(){this.visible =false;}
							,mjhand:function(){this.visible =false;}
							,MJPeng:function(){checkFHLogo(this,1)}
						}
					}
					,_click:function(btn){ showPlayerInfo(1,btn);	 }
					,_event:{
						loadWxHead:function(d){	setWxHead(this,d,1);}
			...
			_event:{
				...
				reinitSceneData:function(eD){ SetPlayerVisible(this,1);},
				...
				newCard:function(eD){HandleNewCard(this,eD,1);},
		...
		top:{
			head:{
				zhuang:{
						_run:function()
						{
							...
							,reinitSceneData:function(){if(CheckArrowVisible()) showPlayerZhuangLogo(this,2);}
						}
					...
					fenghu:
					{
						...
						{
							initSceneData:function(){ checkFHLogo(this,2)}
							,roundEnd:function(){this.visible =false;}
							,mjhand:function(){this.visible =false;}
							,MJPeng:function(){checkFHLogo(this,2)}
						}
					}
			,_click:function(btn){ showPlayerInfo(2,btn);	 }
			,_event:{
					loadWxHead:function(d){	setWxHead(this,d,2);}
			...
			_event:{
				...
				reinitSceneData:function(eD){ SetPlayerVisible(this,2);},
				...
				newCard:function(eD){HandleNewCard(this,eD,2);},
		...
		left:{
			...
			zhuang:{
						_run:function()
						{
							...
							,reinitSceneData:function(){if(CheckArrowVisible()) showPlayerZhuangLogo(this,3);}
						}
					},chatbg:{_run:function(){this.getParent().zIndex = 500;},chattext:{_event:{
					...
					fenghu:
					{
						...
						{
							initSceneData:function(){ checkFHLogo(this,3)}
							,roundEnd:function(){this.visible =false;}
							,mjhand:function(){this.visible =false;}
							,MJPeng:function(){checkFHLogo(this,3)}
						}
					}
			,_click:function(btn){ showPlayerInfo(3,btn);	 }
			,_event:{
					loadWxHead:function(d){	setWxHead(this,d,3);}
			...
			_event:{
				...
				reinitSceneData:function(eD){ SetPlayerVisible(this,3);},
				...
				newCard:function(eD){HandleNewCard(this,eD,3);},
		...
		eat:{
			...
			,_event:{
				...
				reinitSceneData:function(eD){ CheckEatVisible(jsclient.playui.jsBind.eat); }*/
				showcaneat:function(ed){showReplaycaneat(ed)}
			}
		},controllerBg:{
